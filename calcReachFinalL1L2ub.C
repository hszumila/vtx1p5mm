...
double fitZcutUB(double mass){
  ...
  return -168.156+18096*mass-560466*pow(mass,2)+7.33168E6*pow(mass,3)-3.52877E7*pow(mass,4);
...
Double_t CrystalBall(double z, double mass) {
  double par0 = 1.0;
  double par1 = 141.5;
  double par2 = -64.1309+3.96815*mass-0.032729*pow(mass,2);
  double par3 = 8.16594+0.061875*mass;
  double par4 = 0.120411+0.0152718*mass-0.000230058*pow(mass,2);
...
double vertEff(double gct, double zz, double mA){
  //   return 1./gct*exp((-5-zz)/gct)*(-0.36085+0.0241208*mA-0.000276979*pow(mA,2))
  //    *exp(-pow( ( zz-(-57.5354+3.5771*mA-0.0281698*pow(mA,2)))/(2*(3.82466+0.135087*mA)) ,2) );
  //if (mA<24){mA=24;}
  //else if (mA>60){mA=60;}
...
int main(){
  ...
  for (int ii=0; ii<nBinsT; ii++){
    ...
    zCutMax[ii] = zCut[ii]-TMath::Log(1.-quantile);//*5;//vFit->GetParameter(4);
    zCutScaled[ii] = fitZcut(centermass[ii])-TMath::Log(0.1);//*5;//vFit->GetParameter(4);
  ...
  kk3->Close();
   ...
   TCanvas *kk4 = new TCanvas("kk4","par2 for mass",800,800);
  kk4->cd();
