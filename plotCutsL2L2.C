...
int main(){
  ...
  TFile *f = new TFile("../data/hps_L2L2_blind1p5_10Feb17.root");
  TTree *tt=(TTree*)f->Get("ntuple");
  TFile *fout = new TFile("../output_L2L2/cutPlots.root","RECREATE");

  //cuts listed in order
  TCut cut1 = "!eleHasL1&&!posHasL1&&eleHasL2&&posHasL2&&uncP>0.845&&isPair1&&abs(posFirstHitY-100*posPY/posP)<2.5&&abs(eleFirstHitY-100*elePY/eleP)<2.5";
